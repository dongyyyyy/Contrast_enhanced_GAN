...
def training_pix2pix_withPerceptualLoss_all(size=[256,256],blocks=[64,128,256,512,1024],batch_size=128,gpu_num=[0]):
    data_path = '/home/eslab/dataset/Nect CT Png/none_enhanced_png/'
    data_list = os.listdir(data_path)
    data_list.sort()
    data_list  = [data_path + filename + '/' for filename in data_list]
    # print(size[0],size[1])
    save_path = '/data/hdd1/kdy/git/Contrast_enhanced_GAN/saved_model/UNet/batchNorm/perceptualAll_pixel/ReLU/%d_%d_%d_%d_%d_%d_%d_%d/'%(size[0],size[1],batch_size,blocks[0],blocks[1],blocks[2],blocks[3],blocks[4])
    save_img_path = '/data/hdd1/kdy/git/Contrast_enhanced_GAN/saved_img/UNet/batchNorm/perceptualAll_pixel/ReLU/%d_%d_%d_%d_%d_%d_%d_%d/'%(size[0],size[1],batch_size,blocks[0],blocks[1],blocks[2],blocks[3],blocks[4])
    
    logging_path = '/data/hdd1/kdy/git/Contrast_enhanced_GAN/logging/UNet/batchNorm/perceptualAll_pixel/ReLU/'
    logging_filename = logging_path + 'UNet_batchNorm_perceptualAll_pixel_logging_%d_%d_%d_%d_%d_%d_%d_%d.txt'%(size[0],size[1],batch_size,blocks[0],blocks[1],blocks[2],blocks[3],blocks[4])
    ...
    for epoch in range(0,n_epochs):
        ...
        with tqdm(train_loader, desc='Train', unit='batch') as tepoch:
            for index, batch in enumerate(tepoch):
                ...
                with torch.no_grad():
                    p_fake = VGG16(fake_img)
                    p_real = VGG16(real_enhance_image)

                # print(p_fake,p_real)
                perceptual_loss = 0.
                for i in range(len(perceptual_ratio)):
                    perceptual_loss +=  perceptual_ratio[i] * torch.nn.functional.l1_loss(p_fake[i], p_real[i]) 
                loss_G = (gan_ratio * gan_loss) + perceptual_loss + pixel_loss *identity_ratio
